...
$startTime=strtotime(date('Ymd','20150820 00:00:00'))*1000;

if(date('H')==00){
	$endTime=strtotime(date('Ymd'))*1000;
}else {
	$endTime=strtotime(date('Ymd'))*1000+86400000;
}
...
foreach ($alldata as $pf => $pfdata) {
	...
	$one = array();
	$one['date']=date('Ymd',($endTime-86400000)/1000);
	$one['pf'] = "'{$pf}'";
...
foreach ($final as $fieldvalue) {
	...
	$insertSql = "INSERT into cn_regPay_accumulative ($f) VALUES "." ($str) ";
