...
$dump_configs = array(
	'stat_login' => array(
		'src' => 'stat_login',
		'export_fields' => array("date_format(from_unixtime(time/1000),'%Y%m%d')",'uid','time','disconnect',"ifnull(ip,'')",'ifnull(level,0)','ifnull(castlelevel,0)'),
		'import_fields' => array('date','uid','time','disconnect','ip','level','castlelevel'),
		'increment' => 'time',
	),
);
writeRunLog(basename(__FILE__).' start dump stat login with start date  ' . $start_date_ts . ' end date: ' . $end_date_ts);
$yearMonth=array();
if($start_date_ts > 86400 && date('Ym',$start_date_ts/1000)==date('Ym',$end_date_ts/1000)){
