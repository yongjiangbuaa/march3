...
$req_date_start = intval($_REQUEST['start']);;
$req_date_end = intval($_REQUEST['end']);;
...
function calcReg($client,$snapshotdb,$req_date_start,$req_date_end,$tempAppVer)
{
    $sql="select count(1) regCount,u.date,ur.appVersion,r.country,r.pf,r.referrer 
              from $snapshotdb.stat_reg r 
              inner join $snapshotdb.userprofile_full u on r.uid = u.uid 
              inner join $snapshotdb.user_reg ur on r.uid=ur.uid 
      where 
              r.type!=2 
              and u.date >= $req_date_start 
              and u.date<=$req_date_end 
              and u.banTime!=9223372036854775807 
      group by date,appVersion,country,pf,referrer;";
    echo $sql . "\n";
    ...
    foreach( $ret as $value){
        ...
        if (getAppVersionNum($appVersion)<$tempAppVer){
            continue;
        }
        $regArray[$day][$country][$pf][$referrer][$appVersion]= $regCount;
    }
    return $regArray;
}

$regArray = calcReg($client, $snapshotdb, $req_date_start, $req_date_end, $tempAppVer);

$sql="select count(distinct(t.uid)) as perTutCount,t.tutorial,u.date,ur.appVersion,r.country,r.pf,r.referrer 
            from $snapshotdb.stat_tutorial t 
            inner join $snapshotdb.userprofile_full u on t.uid = u.uid 
            inner join $snapshotdb.stat_reg r on t.uid = r.uid 
            inner join $snapshotdb.user_reg ur on t.uid = ur.uid and t.appVersion=ur.appVersion 
      where 
            r.type!=2 and 
            u.date >= $req_date_start and 
            u.date<=$req_date_end and 
            u.banTime!=9223372036854775807 and 
            t.tutorial in($tutorials) 
      group by date,tutorial,appVersion,country,pf,referrer;";
echo "============================"."\n";
echo $sql . "\n";
$ret = query_infobright_new($client,$sql);
$perTutArray=array();
			...
			foreach($rValue as $referrerKey=>$aValue) {
				foreach ($aValue as $appversionKey => $value) {
					foreach ($tutorialIdArray as $tutId) {
						...
						$one['tutorial'] = $tutId;
                        $regCount = intval($regArray[$dateKey][$countryKey][$pfKey][$referrerKey][$appversionKey]);
                        $perCount = intval($perTutArray[$dateKey][$countryKey][$pfKey][$referrerKey][$appversionKey][$tutId]);
                        if ($tutId == 999999999) {
                            $perCount = 0;
                        }else{
                            $regCount = 0;
                        }
                        $one['regCount'] = $regCount;
                        $one['perTutCount'] = $perCount;
						$records[] = $one;
					}
				}
			}
