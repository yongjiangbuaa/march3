...
u.date as regDate,r.country
from $snapshotdb.paylog p inner join $snapshotdb.userprofile u on p.uid = u.uid left join $snapshotdb.stat_reg r on p.uid=r.uid
where  p.date >= $req_date_start and p.date <= $req_date_end
group by regDate,payDate,country order by p.time asc";
...
foreach ($records as $fieldvalue) {
	...
	$db_tbl = "$statdb_allserver.stat_roi_pf_country_v3";
