...
//新服参数
$_REQUEST['db_ip_port'] = $_REQUEST['db_ip_inner'] .':3306';
...
//验证参数、环境
$exist = get_server_info($_REQUEST['server_id']);
if (!empty($exist)) {
	print_r($exist);
	die('server '.$_REQUEST['server_id'].' exists.');
}
if ($exist['ip_inner'] == $_REQUEST['server_ip_inner'] || $exist['ip_pub'] == $_REQUEST['server_ip_pub']) {
	print_r($exist);
	die('server ip '.$_REQUEST['server_ip_inner'].' '.$_REQUEST['server_ip_pub'].' exists.');
}
...
//创建新cokdb
require_once SCRIPTS_DIR.'/create_new_cokdb.php';
...
// //同步badwords  ==>> DONE by java.
// $cmd = '/home/elex/php/bin/php /publish/scripts/sync_badwords.php servers='.$_REQUEST['server_ip_inner'];
// $ret = exec ( $cmd, $out, $status );
// echo_realtime ( "sync_badwords status=".intval($status));
