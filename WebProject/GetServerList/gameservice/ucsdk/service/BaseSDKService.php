...
class BaseSDKService{
    ...
    private static function getServerResponse($serverBaseUrl, $serverPort, $domainServerReqBody, array $params, $prefix, $serviceName){
        try {
            ...
            try {
                if(!empty($domainCache) && !empty($domainCache->ipAddress)){
                    ...
                    $responseBody = HttpClient::quickPost(
                        BaseSDKService::assemblyUrl("http://".$domainCache->ipAddress, $serverPort, $prefix , $serviceName), $requestBody);
                    $sdk_uri = "*******" .BaseSDKService::assemblyUrl("http://" . $domainCache->ipAddress, $serverPort, $prefix, $serviceName);
                }
                else{
                    $responseBody = HttpClient::quickPost(BaseSDKService::assemblyUrl($serverBaseUrl, $serverPort, $prefix , $serviceName), $requestBody);
                    $sdk_uri = BaseSDKService::assemblyUrl($serverBaseUrl, $serverPort, $prefix, $serviceName);
                }
            }catch (Exception $e) {
                $responseBody = false;
            }
            ...
            if($responseObj == null){
                throw new SDKException("请求接口无响应"." url= ".$sdk_uri . " request= ".$requestBody, -1);
            }
