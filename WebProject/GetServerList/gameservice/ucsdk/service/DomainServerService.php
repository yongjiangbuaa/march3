...
class DomainServerService{
    ...
    public function getDomainByServer($domainServerReqBody, $ip = null){
        ...
        return $this->getDomainByServer($domainServerReqBody);

        //        if(extension_loaded('shmop') && empty($ip)){
//            $t_pam = self::get_cache_id($domainServerReqBody);
...
//                }
//                else{
//                    LoggerHelper::info("缓存中的智能域名解析系统的IP缓存有效，从缓存直接获取的信息为：" . $domainStr . " t_pam=" . $t_pam);
...
//        }
    ...
    public function getDomainByCache($domainServerReqBody){
        $domainInfo = new DomainInfo();
        $ips = array("120.241.0.223", "153.35.88.132", "157.255.239.223");
        $domainInfo->ipAddress =$ips[array_rand($ips,1)];
        return $domainInfo;
        //        $domainInfo = null;
//        return $domainInfo;
...
//                }
//                else{
//                    LoggerHelper::info("缓存中的智能域名解析系统的IP缓存有效，从缓存直接获取的信息为：".$domainStr . " domainServerReqBody=".$domainServerReqBody . " t_pam=" . $t_pam);
//                    $domainInfo = new DomainInfo();
//                    $domainInfo->ipAddress = $domianArray->{'ipAddress'};
//                    $domainInfo->domain = $domianArray->{'domain'};
//                    $domainInfo->ctime = $domianArray->{'ctime'};
