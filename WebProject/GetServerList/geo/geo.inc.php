...
require_once GEO_ROOT. '/inc/timezone.php';
require_once GEO_ROOT. '/inc/countrycode.php';

use GeoIp2\Database\Reader;

function geo_get_country_by_ip(&$ip=null, $default=null) {
...
function geo_get_city_by_ip(&$ip=null, $default=null) {
	$geo_ipdbfile = GEO_ROOT.'/GeoIP/GeoLite2-City.mmdb';
	$geo_ok = false;
//	echo $ip."\n";
//	echo $geo_ipdbfile."\n";
	try {
...
function geo_get_timezone_by_ip(&$ip=null, $default=null) {
	$geo_ipdbfile = GEO_ROOT.'/GeoIP/GeoLite2-City.mmdb';
	$geo_ok = false;
	try {
		if ($ip === null) {
		...
		if (!empty($ip)) {
			$geo_reader = new Reader($geo_ipdbfile);
			$record = $geo_reader->city($ip);
			$geo_val = $record->location->timeZone;
			$geo_ok = true;
		}
	} catch (Exception $e) {
		// TODO $e->getMessage();
