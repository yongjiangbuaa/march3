...
class PHPExcel_Writer_Excel2007 implements PHPExcel_Writer_IWriter
{
	 ...
	 * @throws 	Exception
	 */
	public function save4web($pFilename = null , 
	                         $withMicro = false ,
	                         $tmpDir = "./" , 
	                         $excelFileName = "XmlData")
	{
		if ($this->_spreadSheet !== NULL) {
			// garbage collect
			$this->_spreadSheet->garbageCollect();
			
			// excel file have micro in it
			$withMicro = $withMicro && ($this->_spreadSheet->getVbaProject()->getHaveMicro()); 
			
			// If $pFilename is php://output or php://stdout, make it a temporary file...
			$originalFilename = $pFilename;
			if (strtolower($pFilename) == 'php://output' || strtolower($pFilename) == 'php://stdout') {
				$pFilename = @tempnam($tmpDir, $excelFileName);
				if ($pFilename == '') {
					$pFilename = $originalFilename;
				}
			}
			else{
				exit("file name format is invalid");
			}
			...
			$objZip->addFromString('[Content_Types].xml', 			$this->getWriterPart('ContentTypes')->writeContentTypes($this->_spreadSheet, $withMicro));

			// Add relationships to ZIP file
			$objZip->addFromString('_rels/.rels', 					$this->getWriterPart('Rels')->writeRelationships($this->_spreadSheet));
			$objZip->addFromString('xl/_rels/workbook.xml.rels', 	$this->getWriterPart('Rels')->writeWorkbookRelationships($this->_spreadSheet, $withMicro));
			...
			if ($withMicro){
				$vbaPrjContents = null; 
				$vbaPrjZip = new ZipArchive();
				$vbaPrjTarget = $this->_spreadSheet->getVbaProject()->getTarget();
				$vbaPrjTarget = substr($vbaPrjTarget, 6);
				$vbaPrjTargetSplitted = explode('#', $vbaPrjTarget);
				$vbaPrjZip->open($vbaPrjTargetSplitted[0]);				
				$vbaPrjContents = $vbaPrjZip->getFromName($vbaPrjTargetSplitted[1]);
				$vbaPrjZip->close();
				unset($vbaPrjZip);
				$objZip->addFromString($vbaPrjTargetSplitted[1] , $vbaPrjContents);
			}
			...
			// add excle file to zip
			$zipFile = $pFilename . '.zip';
			$excelZip = new ZipArchive;
			if ($excelZip->open($zipFile,ZIPARCHIVE::OVERWRITE) === TRUE) {
				$excelFileInZip = $withMicro? $excelFileName . ".xlsm" : $excelFileName . ".xlsx";
			    $excelZip->addFile($pFilename, $excelFileInZip);
			    $excelZip->addFromString("readme.txt", "Create by php. Elex");
			    $excelZip->close();
			} 
			unset($excelZip);
			@unlink($pFilename);	
	 ...
	 * @throws 	Exception
	 */
	public function save($pFilename = null ,  $withMicro = false)
	{
		if ($this->_spreadSheet !== NULL) {
			// garbage collect
			$this->_spreadSheet->garbageCollect();
			...
			if ($withMicro){
				...
				$vbaPrjTarget = substr($vbaPrjTarget, 6);
				$vbaPrjTargetSplitted = explode('#', $vbaPrjTarget);
				//print_r($vbaPrjTargetSplitted) . "<br />";
				$vbaPrjZip->open($vbaPrjTargetSplitted[0]);				
				$vbaPrjContents = $vbaPrjZip->getFromName($vbaPrjTargetSplitted[1]);
				$vbaPrjZip->close();
				unset($vbaPrjZip);
