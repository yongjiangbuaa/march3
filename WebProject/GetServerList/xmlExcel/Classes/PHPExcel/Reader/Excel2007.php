		...
		foreach ($wbRels->Relationship as $rel) {
			...
			switch ($rel["Type"]) {
				case "http://schemas.microsoft.com/office/2006/relationships/vbaProject": 
					$vbaProject = new PHPExcel_VbaProject();
					$vbaProject->setHaveMicro(TRUE);
					$vbaProject->setTarget( "zip://$pFilename#" . "xl/{$rel['Target']}");
					$excel->setVbaProject($vbaProject);
					break;
			}
		}
										...
										if ($xmlDrawing->twoCellAnchor) {
											foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
												if ($twoCellAnchor->pic->blipFill) {
													...
													$objDrawing->setPath("zip://$pFilename#" . $images[(string) self::array_item($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], false);
													echo "zip://$pFilename#" ;
													echo $images[(string) self::array_item($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")] . "<br />";
