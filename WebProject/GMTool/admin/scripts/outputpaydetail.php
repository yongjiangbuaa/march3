...
global $servers;
$start = strtotime('2015-07-01') * 1000;
$end = strtotime('2015-09-01') * 1000;
$where = "where time >= $start and time < $end and (pf = 'gash' or pf = 'mycard')";
...
// 	writeLog("$server\n");
	...
	for($pageIndex=0;$pageIndex<$pages;$pageIndex++){
		...
		if($result['ret']&&isset($result['ret']['data']))
		{
			...
			{
				$log = $server . $split . implode($split, $sqlData); 
				writeLog($log);
			}
		}else{
...
function writeLog($row){
	file_put_contents( ADMIN_ROOT .'/paylog'.date('Ymd').'.txt', $row . "\n",FILE_APPEND);
}
?>
