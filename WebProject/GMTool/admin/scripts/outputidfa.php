...
global $servers;
$sumsql = "select count(1) sum from stat_reg r inner join userprofile u on r.uid =u.uid where r.time < 1431684000000 and r.pf = 'AppStore' and r.type = 0 and u.gaid != '' and binary upper(u.gaid) = binary u.gaid";
...
//continue;
	...
	for($pageIndex=0;$pageIndex<$pages;$pageIndex++){
		$sql = "select u.gaid from stat_reg r inner join userprofile u on r.uid =u.uid where r.time < 1431684000000 and r.pf = 'AppStore' and r.type = 0 and u.gaid != '' and binary upper(u.gaid) = binary u.gaid order by r.time asc limit " .$pageIndex*$pageLimit. ",$pageLimit";
...
function writeLog($row){
	file_put_contents( ADMIN_ROOT .'/idfalist20150515.txt', $row . "\n",FILE_APPEND);
}
?>
