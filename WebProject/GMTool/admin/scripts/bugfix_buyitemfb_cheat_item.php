<?php
//89374411000019_194_init.dat NOT-EXISTS
...
// 				if (empty($items)) {
					$sqlsel = "select * from user_item where ownerId='$uid'";
					backup_old_data($server,$sqlsel);
					$sql = "delete from user_item where ownerId='$uid'";
					execute_server($server,$sql);
// 				}else{
				foreach ($items as $logrecord) {
// 					$sqlsel = "select * from user_item where ownerId='$uid' and itemId='".$logrecord['itemId']."'";
// 					$currdata = select_old_data($server,$sqlsel);
// 					if (!isset($currdata['itemId'])) {
						$uuid = md5($uid.$logrecord['itemId'].time());
						$sql = "insert into user_item(uuid,ownerId,itemId,count) values ('$uuid', '$uid', '{$logrecord['itemId']}', {$logrecord['count']})";
// 					}else{
// 						$values = array(
// 								'count' => $logrecord['count'],
// 						);
// 						$setsql = build_update_values($values);
// 						$sql = "update user_item set $setsql where ownerId='$uid' and itemId='".$logrecord['itemId']."'";
// 					}
					execute_server($server,$sql);
				}
// 				}
