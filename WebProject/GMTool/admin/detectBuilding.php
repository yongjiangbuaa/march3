...
foreach ($servers as $currentServer=>$content){
    ...
    foreach ($checkArr as $day=>$limit){
        ...
        $sql = "select u.uid,u.name,u.regTime,u.level ulv,(u.gold+u.paidGold) gold,u.gold gameGold,u.paidGold paidGold,u.payTotal payTotal,ub.level blv
        	from userprofile u inner join  user_building ub on u.uid=ub.uid and ub.itemId=400000
        ...
        echo "user_building day $day count ".count($result).PHP_EOL;
...
//	 		$row['说明'] = '>= '.$limit;
	...
	$subject = '异常数据-大本等级-所有服('.date('Y-m-d').')';
	$data = '';
