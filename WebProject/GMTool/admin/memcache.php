...
define('ADMIN_USERNAME','cs'); 	// Admin Username
define('ADMIN_PASSWORD','cs');  	// Admin Password
...
function getCacheItems(){
 ...
 $slabs = sendMemcacheCommands('stats slabs');
 $serverItems = array();
 $totalItems = array();
...
// print_r($slabs);
// echo '</pre>';
 foreach ($slabs as $server => $slablist){
    $freechunks[$server]=0;
    if (!isset($slablist['STAT'])){
        continue;
    }

    $slabinfo = $slablist['STAT'];
    foreach($slabinfo as $keyinfo=>$value){
    	$pos = strpos($keyinfo,':');
    	if($pos === false){
    		continue;
    	}
    	$matches = explode(':',$keyinfo);
        $serverItems[$server][$matches[0]][$matches[1]] = $value;
        if ($matches[1]=='free_chunks'){
            $freechunks[$server] +=$value;
        }
    }
 }
 //print_r($serverItems);
 return array('items'=>$serverItems,'counts'=>$totalItems,'chunks' => $freechunks);
}
function getMemcacheStats($total=true){
	...
	if ($total){
		...
		foreach($resp as $server => $r){
			$res['total_malloced'][$server] = $slabs[$server]['STAT']['total_malloced'];
...
switch ($_GET['op']) {
    ...
    case 1: // host stats
        ...
        $total_evictions = $memcacheStats['evictions'];
...
EOB;
        foreach($MEMCACHE_SERVERS as $server){
            ...
            echo '<tr class=tr-1><td class=td-1>'.$server.'</td><td><a href="'.$PHP_SELF.'&server='.array_search($server,$MEMCACHE_SERVERS).'&op=6" onclick="return confirm(\'确认flush该服务器吗？\')">[<b>Flush this server</b>]</a></td></tr>';
			//echo '<tr class=tr-1><td class=td-1>'.$server.'</td><td>[<b>Flush this server</b>]</a></td></tr>';
    		...
    		echo '<tr class=tr-1><td class=td-0>Total Malloced Size</td><td>',bsize($memcacheStats['total_malloced'][$server]),'</td></tr>';
    		echo '<tr class=tr-0><td class=td-0>Current Items</td><td>',$memcacheStatsSingle[$server]['STAT']['curr_items'],'</td></tr>';
    		echo '<tr class=tr-1><td class=td-0>Total Items</td><td>',$memcacheStatsSingle[$server]['STAT']['total_items'],'</td></tr>';
    		echo '<tr class=tr-0><td class=td-0>Current Connections</td><td>',$memcacheStatsSingle[$server]['STAT']['curr_connections'],'</td></tr>';
    		echo '<tr class=tr-1><td class=td-0>Total Connections</td><td>',$memcacheStatsSingle[$server]['STAT']['total_connections'],'</td></tr>';
    		echo '<tr class=tr-0><td class=td-0>Evictions</td><td>',$memcacheStatsSingle[$server]['STAT']['evictions'],'</td></tr>';
    		echo '</tbody></table>';
	   }
    echo <<<EOB
...
<br/>
	<div class="info"><h2>Cache Information</h2>
		...
		<tr class=tr-1><td class=td-0>Total Evictions</td><td>$total_evictions</td></tr>
		</tbody></table>
		</div>
...
EOB;
			...
			foreach($entries as $slabId => $slab) {
				...
				echo
					...
					"<td class=td-last><b>Item count:</b> ",$slab['number'],
					'<br/><b>Age:</b>',duration($time-$slab['age']),
					'<br/><b>Evicted:</b> ',(isset($slab['evicted'])? $slab['evicted']:'No'),
					'<br/><b>chunk size:</b> ',$slab['chunk_size'],'',
					'<br/><b>chunks per page:</b> ',$slab['chunks_per_page'],
					'<br/><b>total chunks:</b> ',$slab['total_chunks'],
					'<br/><b>used chunks:</b> ',$slab['used_chunks'],
					'<br/><b>free chunks:</b> ',$slab['free_chunks'],
					'<br/><b>free chunks_end:</b> ',$slab['free_chunks_end'],
					'<br/><b>total pages:</b> ',$slab['total_pages'],
					'<br/><b>total memory:</b> ',bsize($slab['chunk_size'] * $slab['total_chunks']);
