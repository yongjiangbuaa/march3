function HotGoods()
{
	...
	this.draw = function(divId){
		...
		.attr("width",wGraph)
		.attr("height",h);
		goods_group = d3.select('#goods').append("svg:svg")
		.attr("width",200)
		.attr("height",h)
		goodsName = goods_group.append("svg:text").attr("y", 50).attr("x", 20);
	};
...
//	更新矩形	
	var updateRec = function(data){
		if(data.length > 0){
			...
			goods_group.selectAll("text").text("商品描述");
			...
			.on("mouseover", function(d,i){
				goods_group.selectAll("text").text(d.goodsId);
			...
			.attr("dy", function(d){return yScale(d.num)+20})
			.attr("dx", function(d,i){return 10 + (i + 0.5)*width;})
			.style("font-size",width * 0.3)
			
			numLabel.enter().append("svg:text")
			.text(function(d){return d.num})
			.transition()
			.attr("dy", function(d){return yScale(d.num)+20})
			.attr("dx", function(d,i){return 10 + (i + 0.5)*width;})
	...
	var clearRecData = function(){
		...
		.attr("height",0)
		.remove();
		goods_group.selectAll("text").text("");
	}
