...
if($_REQUEST['analyze']=='user'){
		...
		//$sql="select count(uid) num from pic_upload_record where stat = 0;";
		$sql="select count(uid) from pic_upload_record where report > 0;";
		...
		//$sql="select u.name, u.picVer, u.uid from pic_upload_record p inner join userprofile u on p.uid = u.uid where stat = 0 limit $index,$page_limit;";
		$sql="select p.report, u.picVer, u.uid, u.name from pic_upload_record p inner join userprofile u on p.uid = u.uid where p.report > 0 and p.stat<2;";
...
// 	}
	...
	for ($i = 0; $i < count($sqlDatas); $i++) {
		...
		$line.="<td><label for='picture_".$sqlDatas[$i]['server']."_{$uid}_{$seq}'><img src='http://coq.eleximg.com/coq/img/$file_folder/$file_name'  style='width:120px;height:120px;' ></label><br>
	...
	$line=substr($line, 5);
	$line.="</tr>";
	$line.="<tr><td colspan='10'><input type='checkbox' id='all_picture' name='all_picture'  onClick='pictureAll()'  />all &nbsp;&nbsp;<input class='btn js-btn btn-primary' type='button' onclick='picConfirm()' id='btn_confirm' name='btn_confirm' value='不处理'>
						&nbsp;&nbsp;<input class='btn js-btn btn-primary' type='button' onclick='picDelete()' id='btn_delete' name='btn_delete' value='修改'></td></tr>";
	$html.=$line;
	$html .= "</table></div><br/>";
	if($pager['pager'])
...
if($_REQUEST['event']=='delete'){
	...
	foreach ($returnUidArray as $uid){
		...
		$sql = "insert into mail (uid, toUser, fromUser, fromName, status, type, rewardStatus, saveFlag, title, contents, createTime, itemIdFlag, reply,srctype) values ('$mailUid', '$uid', '', 'system', 0, 13, 1, 0, '$title', '$contents', $sendTime, 0, 0,105)";
...
if($_REQUEST['event']=='confirm'){
	...
	$sql="update pic_upload_record set report = 0 where uid in ('$uids');";
	$page->executeServer($server, $sql, 2);
	
	adminLogUser($_COOKIE ['u'],'',$server,array('pictureStr'=>$upArray));
	/*$returnUidArray=explode(',', $returnUids);
	foreach ($returnUidArray as $uid){
		$sql="select lang from userprofile where uid='$uid'";
		$result=$page->executeServer ( $server, $sql, 3 );
		...
		$title = addslashes ($lang[105746]);
		$contents = addslashes ($lang[105747]);
