...
// 	foreach($selectServer as $server){
		...
		//$sql="select count(uid) num from userprofile where picVer>0 and picVer<1000000;";
		//$sql="select count(u.uid) num from userprofile u left join user_picture up on u.uid=up.uid where u.picVer>0 and u.picVer<1000000 and (u.picVer>up.picVer or up.picVer is null);";
		...
		$sql="select u.name, u.picVer, u.uid from pic_upload_record p inner join userprofile u on p.uid = u.uid where stat = 0 order by p.updateTime desc limit $index,$page_limit;";
...
if($_REQUEST['event']=='delete'){
	...
	foreach ($returnUidArray as $uid){
		...
		$sql = "insert into mail (uid, toUser, fromUser, fromName, status, type, rewardStatus, saveFlag, title, contents, createTime, itemIdFlag, reply,srctype) values ('$mailUid', '$uid', '', 'system', 0, 13, 1, 0, '$title', '$contents', $sendTime, 0, 0,101)";
