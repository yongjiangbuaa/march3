...
//               $html = '数据为空！';
//            }
	$sql = "select uid, server, banTimes, operator, bandreason ,opTime from banWordRecord where 1=1 ";
	if($start != null){
	...
	if($data != null){
		$html .= '<tr class="listTr"><th>管理员</th><th>用户ID</th><th>服务器Id</th><th>封禁次数</th><th>封禁理由</th>><th>操作时间</th></tr>';
		foreach($data as $each){
			$opTime = date("Y-m-d H:i:s", $each['opTime']/1000);
			$banTimes = $each['banTimes'] + 1;
			$html .= "<tr class='listTr' onMouseOver=this.style.background='#ffff99' onMouseOut=this.style.background='#fff'>".
                    "<td>{$each['operator']}</td><td>{$each['uid']}</td><td>{$each['server']}</td><td>{$banTimes}</td><td>{$each['bandreason']} </td><td>{$opTime}</td></tr>";
		}
		$html .= '</table></div>';
	}else{
		$html .= '</table></div>';
