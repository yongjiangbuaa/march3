...
if($_REQUEST['event']=='output'){
	...
	if($currReferrer&&(!in_array('ALL', $currReferrer))){
		$referrers=implode("','", $currReferrer);
		$whereSql .=" and referrer in ('$referrers') ";
	}
	...
	foreach ($result['ret']['data'] as $curRow){
		...
		if($pf == 'cn1' && ($dateIndesx >='20160825' && $dateIndesx<='20160826')){
			continue;
		}
...
//			$country='合计';
//		}
		if(count($currCountry)>1||in_array('ALL', $currCountry)){
			$country="合计";
		}
	...
	$sql = "select country,pf,date,sum(payTotle) as payTotle,sum(payUsers) as payUsers,sum(dau) as dau,sum(firstPay) as firstPay from stat_allserver.pay_analyze_pf_country_referrer_new $whereSql GROUP BY country,pf,date;";
	$result = query_infobright($sql);
	foreach ($result['ret']['data'] as $row){
		...
		if(in_array($pf,$pf47))
		{
			$total=$total*47.5/100.0;
		}
		if(in_array($pf,$pf48))
		{
			$total=$total*48.75/100.0;
		}
		$eventAll[$dateIndesx][$country]['payTotle'] += $total;
		...
		if (in_array($pf, $currPf)){
			...
			$pfData[$dateIndesx][$country][$pf]['payTotle'] += $total;
	...
	$sql = "select country,pf,date,sum(reg_all) regAll,$fields from stat_allserver.stat_retention_daily_pf_country_referrer_appVersion $whereSql and reg_all>0  group by country,pf,date;";
...
//	print_r($currPf);
	...
	foreach ($result['ret']['data'] as $row){
		...
		foreach($currReferrer as $refValue){
			$eventAll[$dateIndesx]['adcost'] += $row[$refValue];
		}
			...
			switch ($xindex) {
				case '3roi':
					if($forMat<3){
						...
						$eventAll[$dateIndesx][$xindex] = $eventAll[$dateIndesx]['adcost'] ? round($event[$dateIndesx]['3roi'] * 100 / $eventAll[$dateIndesx]['adcost'], 2) . '%' : '-';
					}
					break;
				case '7roi':
					if($forMat<7){
						...
						$eventAll[$dateIndesx][$xindex] = $eventAll[$dateIndesx]['adcost'] ? round($event[$dateIndesx]['7roi'] * 100 / $eventAll[$dateIndesx]['adcost'], 2) . '%' : '-';
					}
					break;
				case '30roi':
					if($forMat<30){
						...
						$eventAll[$dateIndesx][$xindex] = $eventAll[$dateIndesx]['adcost'] ? round($event[$dateIndesx]['30roi'] * 100 / $eventAll[$dateIndesx]['adcost'], 2) . '%' : '-';
					}
					break;
			}
		}
...
function getNameFromNumber($num1) {
	$numeric = $num1 % 26;
...
if($_REQUEST['event']=='user'){
	...
	foreach ($result['ret']['data'] as $curRow){
		...
		if (in_array($pf, $currPf)){
			...
			$pfData[$dateIndesx][$country][$pf]['sdau'] += ($curRow['s_dau'] - $curRow['s_reg']); //老玩家是当天dau-当天新注册
