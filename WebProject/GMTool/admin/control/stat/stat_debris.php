...
if ($_REQUEST['battleType']==0) {
    ...
    foreach ($monthArr as $i) {
        ...
        $sql_debris = "select date, count(l.userid) debris_count from $db_start l WHERE $time and  category=12 and type=1 $whereSql group by date,userid ";
        if($sql_sum){
            $sql_sum = $sql_sum . " union " . $sql_debris ;
        }else{
            $sql_sum = $sql_debris;
        }
    }

    $result_debris = query_infobright($sql_sum);
    $result_debris = $result_debris['ret']['data'];
    $res_array = array();
    foreach ($result_debris as $curRow) {
        $date = $curRow['date'];
        if($date == null){
            continue;
        }
    ...
    if ($_REQUEST['battleType']==1) {
        ...
        foreach ($monthArr as $i) {
            ...
            $sql = "select date ,sum(int_data1) food,sum(int_data2) wood,sum(int_data3) iron,sum(int_data4) crystal from $db_start l WHERE $time
              and  category=12 and type=1 $whereSql group by date,userid ";
            if($sql_sum){
