		...
		foreach ($index as $key=>$title){
			...
			switch ($key){
				case 'alliancename':
					...
					break;
				case 'num':
					$html .= "<td>" . ++$no ."<a href=\"javascript:change_alliname('".$sqlData['uid']."');\">" . '改名' . "</a>". "</td>";
					break;
				case 'abbr':
					$html .= "<td>(" . $value . ")</td>";
					break;
...
if($_REQUEST['dotype'] == 'getmember' && $_REQUEST['alUid']){
	...
	$sql = "select u.*,p.*,ur.*,ub.level ubLevel,am.rank alrank from {$className} inner join alliance_member am on u.uid=am.uid where am.allianceId='$alUid';";
	$result = $page->execute($sql,3);
		...
		foreach ($index as $key=>$title){
			...
			switch ($key){
				case 'uid':
					...
					break;
				case 'alrank':
					$html .= "<td><a href=\"javascript:edit('".$user."');\">" . ($value == 5 ? '盟主' : 'R'.$value). "</a></td>";
					break;
				case 'gender':
...
if($_REQUEST['dotype'] == 'changename' && $_REQUEST['alUid'] && $_REQUEST['alliname']) {
	$alUid = trim($_REQUEST['alUid']);
	$alliname = trim($_REQUEST['alliname']);
	$sql = "update alliance set alliancename='$alliname' where uid='$alUid'";
